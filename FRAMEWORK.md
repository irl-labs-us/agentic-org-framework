...
**Solo-operator vs. multi-operator: pick one at onboarding, don't leave it implicit.** Everything below assumes a real second party — another human, or another operator's agent — who could plausibly hold write access at the same time. That's exactly the condition README.md's "When this doesn't fit" section flags: RoleWise itself later removed this framework's CEO/CSO org-simulation layer (mission packets, the evaluation covenant, the coordination ceremony) because it never actually had a second decision-maker to coordinate with. The git discipline below earns its keep differently than that removed layer did, though — it's a mechanically-enforced CI check that caught a real, recurring failure (branches drifting from the base without reconciling), not organizational roleplay, and that failure is just as live for one operator running several parallel agent sessions as for a multi-person team. So the split here is narrower than "adopt or skip the whole thing":

- **Multi-operator mode** (the default described below): Merge Steward is a real role with delegation, and the live Git-work lease ledger exists because there is genuinely more than one party who might hold write access at once.
- **Solo-operator mode**: drop the lease ledger and Merge Steward delegation ceremony (nobody to delegate to or from — {CEO} is permanently the only merge authority), but keep every mechanically-enforced check unconditionally: single-use branches from a fresh exact base SHA, the changed-file manifest matching the real diff, the ban on undeclared merge commits, and high-risk-scope classification. `scripts/check_git_governance.py`'s `SOLO_MODE` flag (set once at onboarding) implements exactly this split — it removes only the lease-ledger requirement from CI, nothing else. Full detail and the exact adoption steps live in `templates/GIT_OPERATIONS_COVENANT.md`'s "Solo-operator mode" section.

Do not make the mechanical checks themselves configurable per project beyond this one fork — a rule that catches real drift shouldn't become a menu of options, or a project will end up re-deriving the RoleWise git-drift incident this discipline exists to prevent, just with extra steps to disable the part that would have caught it.
