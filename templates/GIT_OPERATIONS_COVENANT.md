...
2. Build the pull-request body from `.github/pull_request_template.md`; retain and complete the machine-readable metadata contract below. Before publishing, re-run `scripts/check_pr_readiness.py` with `--body-file <path-to-drafted-body>` (and `--solo-mode` if this project has adopted it) — this validates the body locally the exact way `check_git_governance.py` will in CI (required headings present, every Changed-file manifest line well-formed, declared manifest matching the real diff). A real incident this caught after the fact: a manifest bullet with a trailing parenthetical after the closing backtick (e.g. `` - `path` (added for X) ``) looks correct on a skim, but fails `check_git_governance.py`'s exact-match parser silently — the whole line is dropped rather than partially matched, so the path is reported as "missing from manifest" even though it's visibly present in the body. Catch this locally before pushing, not as a confusing CI failure afterward.
...
`## Authorized scope` is a separate authorization boundary, not another copy of the manifest. It requires one meaningful scope reference and one or more `- Approved path: ` entries. An entry may be an exact path or a directory prefix ending in `/**`; blanket wildcards are prohibited. CI reports changed files outside this boundary separately from manifest errors so scope expansion cannot be hidden by refreshing the manifest.

`## Risk and review evidence` records risk class, reviewer identity, independence declaration, decision, reviewed head SHA, policy/config version, evidence references, decision timestamp, and optional expiry. The decision must be `approve` for merge validation. High-risk work requires `Risk class: high`, an `independent` declaration, and a reviewer different from the configured principal. Every approval is bound to the exact candidate head and becomes stale after a push. CI validates the record's structure, allowed values, candidate binding, and expiry; the named reviewer and Merge Steward remain responsible for the truth, relevance, and quality of the evidence.

When the head changes, the author updates the current reviewed SHA, manifest, risk classification, tests, and evidence before requesting another gate. Authors using the GitHub CLI must fill a temporary copy of `.github/pull_request_template.md` and pass it with `gh pr create --body-file <completed-file>`; abbreviated `--body`, `--fill`, or free-form bodies are prohibited because they bypass required metadata. Before publication, compare the manifest with `git diff --name-only <recorded-base>..HEAD` and run the local readiness/governance checks. Missing or renamed headings, placeholder-only required content, misplaced lease metadata, an absent numeric live-ledger grant-comment link, a stale review SHA, out-of-scope files, or any manifest mismatch is a preventable authoring failure and CI must fail closed.
...
### Keeping the Changed-file manifest from going stale

The exact-match manifest requirement is a real safety property (it's what actually catches a PR body that doesn't match what's being merged) but it means any open PR's manifest goes stale the instant another commit lands on its branch, failing the covenant check again until someone edits the body. Two failure modes, two different fixes:

**Before you push at all: a malformed manifest line, not staleness.** `scripts/check_pr_readiness.py --body-file <path> [--solo-mode]` (Required preflight, above) parses your drafted body locally with the exact same regex `check_git_governance.py` uses in CI, and names the specific bad line rather than a generic mismatch — this is the cheap, always-available first line of defense, and requires no CI setup at all.
...
Both synchronizers use the same bounded sequence: fetch and compute against exact refs, read the live body and head together, retry once if either changes, update only owned mechanical sections, verify the head again, and dispatch the trusted pull-request governance run for that exact SHA. They rerun a completed exact-head run regardless of its prior conclusion because `GITHUB_TOKEN` body edits do not emit a new pull-request event. If GitHub has not materialized an exact-head run yet, the command reports that precise bounded-pending state. It never dispatches workflow code from the candidate branch.

**This only works safely if `git-governance.yml`'s concurrency group is SHA-scoped, not just per-PR** (see that workflow's own `concurrency:` block) — otherwise a stale rerun queued after a legitimate run for a newer commit has already started would `cancel-in-progress` the real, current-head evaluation, producing a phantom `cancelled` status on the actual current head rather than the rerun simply resolving its own stale state.
