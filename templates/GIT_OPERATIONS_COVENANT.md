...
- **`scripts/create_release_pr.py --sync-mechanical`**, run on every push to `staging` (e.g. a `release-pr-sync.yml` workflow triggered `on: push: branches: [staging]`). For the persistent release PR, `## Branch integration` and `## Changed-file manifest` are both fully mechanical (no human checkboxes), so both are safely regenerated; `## Outcome`, `## Coordination and scope`, and `## Evidence` are preserved verbatim from whatever a human last wrote (via `body_section()`, which extracts a named `## Heading` section's raw text).
...
**A rerun step is required for either automation to actually resolve a stale failure**, not just an optional nicety: editing a PR body via `gh pr edit` with the ambient `GITHUB_TOKEN` does not trigger a new `pull_request` "edited" event — GitHub explicitly suppresses workflow-triggering for anything done by the default token, to prevent infinite loops. Without an explicit rerun, `git-governance.yml`'s last (failing) run for that PR sits stale forever, even after the manifest that caused it is fixed. Both scripts' `rerun_stale_check()` finds the most recent `git-governance.yml` run for the current head SHA via `gh run list` and re-runs it via `gh run rerun` if its conclusion was `failure` (requires `actions: write` on top of the `pull-requests: write` the body edit itself needs); a rerun failure (e.g. the run aged out of GitHub's ~30-day rerun window) is caught and reported as a note rather than turning an otherwise-successful manifest sync into a reported failure.
